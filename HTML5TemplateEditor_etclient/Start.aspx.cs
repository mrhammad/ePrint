...
namespace HTML5TemplateEditor_etclient
{
    ...
    {
        //protected PlaceHolder plhTemplates;

        //protected HtmlGenericControl Templatesdiv;
        ...
        public void GeneratePlhContent(string host)
        {
            ...
            if (str6 != null)
            {
                ...
                {
                    str5 = "94643DBB-7367-4CAE-A4D3-E9857995D";
                }
            ...
            foreach (DataRow row in dataSet.Tables[0].Rows)
            {
                ...
                object[] objArray = new object[] { "tid=", row["TemplateID"].ToString(), "&SessionID=", this.Session.SessionID, "&companyid=", row["CompanyID"].ToString(), "&CartItemID=", 9943, "&ID=", row["PriceCatalogueID"].ToString(), "&StoreUserID=", 1403, "&dbkey=", str5, "&ctype=b2b&AccountID=", 7 };
                string str9 = Encrypt.Encryption(string.Concat(objArray));
                this.plhTemplates.Controls.Add(new LiteralControl(string.Concat("<a id='Name' href='editabletemplate.aspx?", str9, "' class='TemplateDiv' style='text-decoration:none;'>")));
