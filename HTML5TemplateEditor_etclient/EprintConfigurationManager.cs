...
public class EprintConfigurationManager
{
	...
	public DataSet GetAppSettingsAndConnectionString()
	{
		...
		string str = string.Concat("select * from tb_Template_AppSettings where ServerName='", TemplateEditorFrontEndSevice.HostName, "'");
		str = string.Concat(str, ";  select * from tb_Template_ConnectionStrings where ServerName='", TemplateEditorFrontEndSevice.HostName, "'");
