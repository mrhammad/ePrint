...
(function (window, document) {
    ...
    Croppic.prototype = {
        ...
        initialScaleImg: function () {
            ...
            //Gaj
            // $("#Imageeditor1").width(that.imgW).height(that.imgH);

            //$("#controlsDiv").css({ 'left': -(that.imgW - that.objW) / 2, 'top': -(that.imgH - that.objH) / 2, 'position': 'absolute' });
            that.img.css({ 'left': -(that.imgW - that.objW) / 2, 'top': -(that.imgH - that.objH) / 2, 'position': 'absolute' });
            if (that.options.imgEyecandy) { that.imgEyecandy.css({ 'left': -(that.imgW - that.objW) / 2, 'top': -(that.imgH - that.objH) / 2, 'position': 'relative' }); }



            //$('#divCropMain2').css({ 'width': 'auto' });
            //$('#divCropMain3').css({ 'width': 'auto' });

            //alert($('#divCropMain2').outerWidth());
            //alert($('#divCropMain2').position().left);
            //alert($('#cropContainerModal').position().left);

            //alert($('#cropContainerModal').position().left);



            var ImageDivLeft = parseInt($('#cropContainerModal').position().left);
            var ImageDivWidth = parseInt($('#cropContainerModal').outerWidth());
            var divCropMainWidth = parseInt($('#divCropMain2').outerWidth());
            ImageDivLeft = parseInt($("div[aria-describedby=Imageeditor1]").width());


            //alert((ImageDivLeft + (ImageDivWidth / 2)) - divCropMainWidth/2 );




            $('#divCropMain1').css({ 'left': (ImageDivLeft / 2) - divCropMainWidth / 2 });
            $('#divCropMain2').css({ 'left': (ImageDivLeft / 2) - divCropMainWidth / 2 });
            $('#divCropMain3').css({ 'left': (ImageDivLeft / 2) - divCropMainWidth / 2 });



            //$('#divCropMain1').css({ 'left': (ImageDivWidth / 2) });
            //$('#divCropMain2').css({ 'left': (ImageDivWidth / 2)});

            //$('#divCropMain3').css({ 'left': (ImageDivWidth / 2) });



            $('#divCropMain1').css({ 'top': 0 });
            $('#divCropMain2').css({ 'top': 23 });
            $('#divCropMain3').css({ 'top': 102 });
            $('#divCropMain2').css({ 'width': divCropMainWidth });
            $('#divCropMain3').css({ 'width': divCropMainWidth });
        },
        ...
        createCropControls: function () {
            ...
            //  html = '<div id="divCropMain1"  class="cropControlsTop cropControlsCrop"><i class="cropControlTitle">Drag image to crop</i></div>';

            //html += '<div id="divCropMain2"  style="clear:both;" class="cropControls cropControlsCrop">' + cropControlZoomMuchIn + cropControlZoomIn + cropControlZoomOut + cropControlZoomMuchOut + cropControlRotateLeft + cropControlRotateRight + '</div>';

            //html = '<div id="divCropMain3" style="clear:both;" class="cropControlsBelow cropControlsCrop">' + cropControlCrop + cropControlReset + '</div>';

            // that.obj.append(html);

            // $('#Imageeditor1').append(html); 



            //   that.cropControlsCrop = that.obj.find('.cropControlsCrop');

            that.cropControlsCrop = $('#Imageeditor1');

            // CACHE AND BIND CONTROLS
            if (that.options.doubleZoomControls) {
                that.cropControlZoomMuchIn = that.cropControlsCrop.find('.cropControlZoomMuchIn');
                that.cropControlZoomMuchIn.unbind("click").on('click', function () { that.zoom(that.options.zoomFactor * 10); });

                that.cropControlZoomMuchOut = that.cropControlsCrop.find('.cropControlZoomMuchOut');
                that.cropControlZoomMuchOut.unbind("click").on('click', function () { that.zoom(-that.options.zoomFactor * 10); });
            }

            that.cropControlZoomIn = that.cropControlsCrop.find('.cropControlZoomIn');
            that.cropControlZoomIn.unbind("click").on('click', function () { that.zoom(that.options.zoomFactor); });

            that.cropControlZoomOut = that.cropControlsCrop.find('.cropControlZoomOut');
            that.cropControlZoomOut.unbind("click").on('click', function () { that.zoom(-that.options.zoomFactor); });

            that.cropControlZoomIn = that.cropControlsCrop.find('.cropControlRotateLeft');
            that.cropControlZoomIn.unbind("click").on('click', function () { that.rotate(-that.options.rotateFactor); });

            that.cropControlZoomOut = that.cropControlsCrop.find('.cropControlRotateRight');
            that.cropControlZoomOut.unbind("click").on('click', function () { that.rotate(that.options.rotateFactor); });

            that.cropControlCrop = that.cropControlsCrop.find('.cropControlCrop');
            that.cropControlCrop.unbind("click").on('click', function () { that.crop(); });

            that.cropControlReset = that.cropControlsCrop.find('.cropControlReset');
            that.cropControlReset.unbind("click").on('click', function () { that.reset(); });

            //if ($(".cropImgWrapper").height() + $('#divCropMain1').height() + $('#divCropMain2').height() + $('#divCropMain3').height() > $("#Imageeditor1").height())
            //    $("#Imageeditor1").css({ 'overflow-y': 'auto' });
        ...
        crop: function () {
            ...
            // $(that.cropControlsCrop).dialog("close");/*added by shahbaz for Closing Whole Window after Finish Click*/
            that.showLoader();
            ...
            //    originalFilename = Control.EditedImageName;
            //originalFilename = Control.ImgUrl;// To Show last cropped image by Gaj
            //alert(FrontEndUploadPath);

            // alert(originalFilename);
            var array = originalFilename.split('.');
            var GUID = Guid();
            var newFilename = array[0] + "-" + GUID.substr(0, 3) + "." + array[1];
            ...
            var cropData = {
                ...
                FrontEndUploadPath: FrontEndUploadPath,
            ...
            $.ajax({
                ...
                if (response.status == 'success') {
                    ...
                    that.init();
                    $(".loading_new").show();
                    FitTheEditedImageToControl(newFilename);

                    $("#Imageeditor1").dialog("close");
                    that.hideLoader();
                }
                if (response.status == 'error') {
                    if (that.options.onError) that.options.onError.call(that, response.message);
        ...
        destroy: function () {
            ...
            if (!$.isEmptyObject(that.cropControlsCrop)) { $(that.cropControlsCrop).dialog("close"); that.cropControlsCrop.hide(); }/*changed by shahbaz for Closing Whole Window after Finish Click*/
