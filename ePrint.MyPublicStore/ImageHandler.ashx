...
public class ImageHandler : IHttpHandler
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        if (_httpContext.Request.QueryString["fromattachment"] != null)
        {
            FromStoreAttach = _httpContext.Request.QueryString["fromattachment"];
        }
        ...
        if (ConnectionClass.SecureDocFilepath != null)
        {
            if (FromStoreAttach.ToLower() == "false")
            {

                artworkPath = ConnectionClass.SecureDocFilepath + ConnectionClass.ServerName + "/" + _CompanyID + "/attachments/";
            }
            else
            {
        ...
        if (File.Exists(_fileName))
        {
            ...
            {
                FileInfo objFile = new FileInfo(_fileName);
                byte[] content = File.ReadAllBytes(_fileName);
                context.Response.StatusCode = 200;
                context.Response.ContentType = "application/octet-stream";
                context.Response.Buffer = true;
                context.Response.Clear();
                context.Response.AddHeader("content-disposition", "attachment;filename=\"" + objFile.Name + "\"");
