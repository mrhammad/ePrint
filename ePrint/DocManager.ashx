<%@ WebHandler Language="C#" Class="DocManager" %>
...
public class DocManager  : IHttpHandler, IRequiresSessionState
{
			...
			switch (str)
			{
				...
				case "product_ftp":
				{
					if (!string.IsNullOrEmpty(arrayLists[3].ToString()))
						...
						DataTable dataTable1 = new DataTable();
						dataTable1 = this.comncls.GetFileNameonDocType("product_ftp", Convert.ToInt32(this._PID));
						IEnumerator enumerator2 = dataTable1.Rows.GetEnumerator();
						try
						{
							while (enumerator2.MoveNext())
							{
								DataRow current1 = (DataRow)enumerator2.Current;
								this.SecFileName = current1["FTPFile"].ToString();
								if (string.IsNullOrEmpty(this.SecFileName))
								{
									continue;
								}
