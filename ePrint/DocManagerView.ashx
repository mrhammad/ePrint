<%@ WebHandler Language="C#" Class="DocManagerView" %>
...
public class DocManagerView  : IHttpHandler, IRequiresSessionState
{
	...
	public void ProcessRequest(HttpContext context)
	{
		...
		if (lower != null)
		{
			switch (str)
			{
				...
				case "imgcpstripe":
				{
					string[] secureDocPath3 = new string[] { this.SecureDocPath, this.ServerName, "/", this._CompanyID, "/Stripe/", this.ReqSecuredFileName };
					this.path = string.Concat(secureDocPath3);
					break;
				}
				case "stripenew":
				{
					string[] strArrays = new string[] { this.SecureDocPath, this.ServerName, "/", this._CompanyID, "/Stripe/", this.ReqSecuredFileName };
					this.path = string.Concat(strArrays);
					break;
				}
				case "attachments":
...
context.Response.ContentType = "image/" + fileInfo.Extension.TrimStart('.').ToLower(); // e.g., image/png or image/jpeg

// Allow the browser to display the image inline
context.Response.AddHeader("Content-Disposition", "inline;filename=\"" + fileInfo.Name + "\"");

// Transmit the file
context.Response.TransmitFile(this.path);
