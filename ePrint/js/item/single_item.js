    ...
    for (var i = 0; i < arr1.length; i++) {
        ...
        if (estimateType == "digital") {
            if (arr2[0] == ddlValue) {
                ...
                //To set the Apply Press Restrictions
                var printimageheightrestricted = arr2[8].split('»');
                var printimagewidthrestricted = arr2[9].split('»');
                var PrintImagesideAreaHeight = printimageheightrestricted[1];
                var PrintImagesideAreaWidth = printimagewidthrestricted[1];
                if (PrintImagesideAreaHeight > 0 || PrintImagesideAreaWidth > 0) {
...
function SendPaperIDandName(id, values, papertype, MaterialNo, Papeweight) {
    lblDefaultPaper.title = SpecialDecode(values);
    lblDefaultPaper.innerHTML = SpecialDecode(trim12(values));
    lblPaperWeight.innerHTML = Papeweight;
...
function setwidth() {
    ...
    if (is_ie) {
        document.getElementById("div_GuttersCustomSize").style.width = "222px";
    }
    else {
        document.getElementById("div_GuttersCustomSize").style.width = "222px";
    }
...
function GuillotineSelect() {
    ...
    window.radopen(strSitepath + "common/common_popup.aspx?type=moreplant&pg=estimate",null, '1000', '500');
    SetRadWindow('divrad', 'divBackGroundNew', '200');
...
function LoadToSheetCustom(ddlValue) {
    ...
    if (ddlValue == "0") {
        ...
        for (var i = 0; i < arr1.length; i++) {
            var arr2 = arr1[i].split('±');
            if (ddlValue == Number(arr2[0])) {
                txtsectionheight.value = Eprint_ReturnFinal_Formated_Amount(CompanyID, UserID, Number(arr2[2]), decimalPlaces, '', false, false, false);
                txtsectionwidth.value = Eprint_ReturnFinal_Formated_Amount(CompanyID, UserID, Number(arr2[3]), decimalPlaces, '', false, false, false);
            }
        }
    }
}
...
function PrintSheetCustom(chID) {
    ...
    if (chCheck) {
        ...
        //txtsectionheight.focus(); TicketID --> 13027
    }
    else {
...
function JobItemCustom(chID) {
    ...
    if (chCheck) {
        ...
        document.getElementById("div_ddlJobItemSize").style.display = "none";
        //txtitemheight.focus(); TicketID --> 13027
    }
    else {
...
function GuttersCustom(chID) {
    ...
    if (chCheck) {
        document.getElementById("div_GuttersCustomSize").style.display = "block";
        //txtGutterHorizontal.focus(); TicketID --> 13027
    }
    else {
        document.getElementById("div_GuttersCustomSize").style.display = "none";
    }
            ...
            if (productType == "singleitem") {
                ...
                if (IsItOk && IsBookletValid) {
                    if (ValidatePaper_HeightWidth()) {
                        ...
                        if (RequestType == "add" || RequestType == "more" || RequestType == "sub") {
                            var desin = '';
                            for (var i = 0; i < bookArr.length; i++) {
...
function CreateItemValidation() {
    ...
    if (!CheckDecimalPlus(txtRunningSpoilage.value, 'spn_txtRunningSpoilage', 'spn_txtRunningSpoilage', 'yes')) {
        IsCorrect = false;
    }

    if (!CheckDecimalPlus(txtsectionheight.value, 'spn_PrintSheetCustomSize', 'spn_PrintSheetCustomSize_value', 'yes')
        || !CheckDecimalPlus(txtsectionwidth.value, 'spn_PrintSheetCustomSize', 'spn_PrintSheetCustomSize_value', 'yes')) {
        IsCorrect = false;
    }

    if (!CheckDecimalPlus(txtitemheight.value, 'spn_JobItemCustomSize', 'spn_JobItemCustomSize_Value', 'yes')
        || !CheckDecimalPlus(txtitemwidth.value, 'spn_JobItemCustomSize', 'spn_JobItemCustomSize_Value', 'yes')) {
        IsCorrect = false;
    }
    if (!CheckDecimalPlus(txtGutterHorizontal.value, 'spn_txtGutterHorizantal', 'spn_txtGutterHorizantal_value', 'yes')
         || !CheckDecimalPlus(txtGutterVertical.value, 'spn_txtGutterHorizantal', 'spn_txtGutterHorizantal_value', 'yes')) {
        IsCorrect = false;
    }
...
function Calculations() {
    G_CalculateLandscape("s");
    G_CalculatePortrait("s");
    if (txtManual.value == "") {
        txtManual.value = 0;
    }
    ...
    if (Number(result_port.value) > Number(result_land.value)) {
        if (SingleItemPrintLayout == '') {
            chkPortrait.checked = true;
            chkLandscape.checked = false;
            chkManual.checked = false;
        }
        else if (SingleItemPrintLayout.toLowerCase() == 'p') {
            chkPortrait.checked = true;
            chkLandscape.checked = false;
            chkManual.checked = false;
            SingleItemPrintLayout = '';
        }
        else if (SingleItemPrintLayout.toLowerCase() == 'l') {
            chkPortrait.checked = false;
            chkManual.checked = false;
            chkLandscape.checked = true;
            SingleItemPrintLayout = '';
        } else if (SingleItemPrintLayout.toLowerCase() == 'm') {
            chkPortrait.checked = false;
        ...
        else if (SingleItemPrintLayout.toLowerCase() == 'p') {// for bugid : 1651
            chkPortrait.checked = true;
            chkLandscape.checked = false;
            chkManual.checked = false;
            SingleItemPrintLayout = '';
            ...
            for (var i = 0; i < arr_1.length; i++) {
                ...
                if (arr_2[0] == "EstimateType") {
                    ...
                    for (var j = 0; j < qty_1.length; j++) {
                        var qty_2 = qty_1[j].split("" + GetCurrencyinRequiredFormate("", true) + "");
                        if (qty_2[0] == "M") {
                    ...
                    lblGuillotine.innerHTML = arr_2[2].length > 25 ? SpecialDecode(arr_2[2].substring(0, 23)) + "..." : SpecialDecode(arr_2[2]);
                }
        ...
        if (arr_0[b] != '') {
            ...
            for (var i = 0; i < arr_1.length; i++) {
                ...
                if (arr_2[0] == "EstimateType") {
                    ...
                    PressOnChange(ddlPress); //by kumar,bec nonheight and nonwidth is coming 0 in case of edit,now it is correct after putting this line.
                }
                else if (arr_2[0] == "Paper") {
                    hdnpaperID.value = arr_2[1];
                    ////lblDefaultPaper.innerHTML = arr_2[2].length > 25 ? arr_2[2].substring(0,23)+"..." : arr_2[2];                
            ...
            if (document.getElementById("spn_FromQty_" + i + "") != null) {
                ...
                if ((Number(val) >= Number(FromQty)) && (Number(val) <= Number(ToQty))) {
                    ...
                    if (i == Number(Max) - Number(1)) {
                        ...
                        document.getElementById("spn_QtyPrice_" + index + "").innerHTML = GetCurrencyinRequiredFormate("0.00", true);
                    }
                }
            }
        }
...
function SendGuillotineIDandName(id, values, FirstTrim, SecondTrim) {
    ...
    //        lblGuillotine.innerHTML = lblGuillotine.innerHTML.substring(0, 50) + "...";
    //    }
...
function DefaultLanding() {
    var chkSheetSize = document.getElementById("ctl00_ContentPlaceHolder1_UCSingleItem_chkPrintSheet");
    var chkJobSize = document.getElementById("ctl00_ContentPlaceHolder1_UCSingleItem_ChkJobFlatCustom");
    document.getElementById("div_ddlPrintSheetSize").style.display = "none";
    document.getElementById("div_ddlJobItemSize").style.display = "none";
