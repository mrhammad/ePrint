...
function OpenPaperPopUp(Type) {
    if (Type == 'paper') {
        ...
        window.radopen(strSitepath + "common/common_popup.aspx?type=invselector&pg=estimate&item=paper&papertype=" + papertype + "", '950', '430');
...
function popup_phrasebook(type) {
    alert(type);
    if (Estimationtype != 'O') {  
        // PopupCenter(strSitepath + "common/phrase_book.aspx?type=Estimate " + type, '800', '400')
        var RadWindow_Desn = window.radopen(strSitepath + "common/phrase_book.aspx?pg=" + Pagetype + "&type=Estimate " + type, '800', '400');
        //SetRadWindow('divrad', 'divBackGroundNew', '200');
        SetRadWindow_Ver2('divrad', 'divBackGroundNew');
        RadWindow_Desn.setSize(866, 450);
        RadWindow_Desn.center();

    }
    else {        
        // PopupCenter(strSitepath + "common/phrase_book.aspx?type=PrintBroker " + type, '800', '400')
        var RadWindow_Desn = window.radopen(strSitepath + "common/phrase_book.aspx?pg=" + Pagetype + "&type=PrintBroker " + type, '800', '400');
        // SetRadWindow('divrad', 'divBackGroundNew', '200');
        SetRadWindow_Ver2('divrad', 'divBackGroundNew');
        RadWindow_Desn.setSize(866, 450);
        RadWindow_Desn.center();
    ...
    for (var i = 0; i < ArrayPrint.length; i++) {
        ...
        for (k = 0; k < pirArr.length; k++) {
            if (pirArr[k].Quantity != null && pirArr[k].Quantity != '') {
                ...
                StoreData += "QuantityNumber »" + pirArr[k].QuantityNumber + 'Ã‡';
            }
        }
        StoreData += "µ";
    }
...
/* //=== COMMENTED BY SWETHA ON 25/5/2010 Moved to othercost_item.js =====//
//===******* OTHERCOST SECTION *********==//
var OtherIndex = '';
...
function OpenTaskPage(OnorOff) {
    if (OnorOff) {
        if (document.getElementById("div_TaskUC").style.display == "block") {
            document.getElementById("div_TaskUC").style.display = "none";
            document.getElementById("ctl00_ContentPlaceHolder1_UCItemDescription_chk_FollowupTask").checked = false;

        }
        else {
            // setTaskPopUpCenter(document.getElementById("div_task_add"));
            document.getElementById("div_TaskUC").style.display = "block";
            showDivPopupCenter('div_task_add', '300'); //UC estimateadd page id to be taken 
            if (TaskAddData == '') {
            ...
            document.getElementById("ctl00_ContentPlaceHolder1_UCItemDescription_chk_FollowupTask").checked = true;
...
function SendPhraseBook(id, values, phrasetype, tooltip) {
    ...
    try {
        var phrasetypecommon = phrasetype.toString().split(' ')[0];
    } catch (Error) { }
    if (tempEstimateType == '') {
    ...
    if (tempEstimateType == 'warehouse') {
        if (phrasetype == '' + phrasetypecommon + ' Title') {
            txtWareItemTitleID.value = values;
        }
        if (phrasetype == '' + phrasetypecommon + ' Artwork') {
            txtWareArtwork.value = values;
        }
        if (phrasetype == '' + phrasetypecommon + ' Description') {
            txtWareDesign.value = values;
        }
        if (phrasetype == '' + phrasetypecommon + ' Colours') {
            txtWareColour.value = values;
        }
        if (phrasetype == '' + phrasetypecommon + ' Size') {
            txtWareSize.value = values;
        }
        if (phrasetype == '' + phrasetypecommon + ' Material') {
            txtWareMaterial.value = values;
        }
        if (phrasetype == '' + phrasetypecommon + ' Delivery') {
            txtWareDelivery.value = values;
        }
        if (phrasetype == '' + phrasetypecommon + ' Finishing') {
            txtWareFinishing.value = values;
        }
        if (phrasetype == '' + phrasetypecommon + ' Proofs') {
            txtWareProofs.value = values;
        }
        if (phrasetype == '' + phrasetypecommon + ' Packing') {
            txtWarePacking.value = values;
        }

        if (phrasetype == '' + phrasetypecommon + ' Notes') {
            txtWareNotes.value = values;
        }
        if (phrasetype == '' + phrasetypecommon + ' Terms') {
