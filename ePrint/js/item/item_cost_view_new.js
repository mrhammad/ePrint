...
function ItemMarkupOnBlur_ware(Markup, ID, MinimumCost) {
    ...
    if (ID != "" && !isNaN(Markup)) {
        ...
        var SellInMarkValue = Eprint_GetminimumCost_ComparedtoCostwithMarkup_ForOtherCost(SellExMarkupValue, Markup, MinimumCost, '', '', 0);
...
function ItemMarkupOnBlur(Markup, ID, MinimumCost, Setupcost) {
    if (ID != "" && !isNaN(Markup)) {
        ...
        //var SellInMarkValue = Number(SellExMarkupValue) + (Number(SellExMarkupValue) * Number(Markup) / 100); 
        var SellInMarkValue = Eprint_GetminimumCost_ComparedtoCostwithMarkup_ForOtherCost(SellExMarkupValue, Markup, MinimumCost, '', '', Setupcost);
        SellInMarkValue = SellInMarkValue;
        if (MinimumCost != 0) {
            if (SellInMarkValue >= MinimumCost) {
                ...
                //SetupCost was removed because it is already added to SellInMarkvalue - no need to add it once again.
                SellInMarkValue = Number(SellInMarkValue);
            }
            else {
        ...
        for (var i = 0; i < 4; i++) {
            ...
            if (MarkupPercentage != "" && !isNaN(MarkupPercentage)) {
                if (document.getElementById(spnPriceId) != null) {
                    ...
                    //var MarkupValue = (markup * price) / 100;   
                    //var SellPrice = Number(MarkupValue) + Number(price);

                    var price1 = price.replace(/,/g, '');            // By Nat
                    var markup1 = markup.replace(/,/g, '');
                    var Mark_Mul = (price1 * markup1);
                    var Mark_Div = Mark_Mul / 100;

                    var SellPrice = Number(Mark_Div) + Number(price1);
                    SellPrice = roundNumber(SellPrice, 2);                     //End
...
function CataloguePriceOnBlur(txtobj,markupValue,index) {
    debugger
    var MarkupPercentage = markupValue;
    var CostPrice = txtobj.value;
    //MarkupPercentage = roundNumber(Number(MarkupPercentage), 2)
    MarkupPercentage = Eprint_ReturnFinal_Formated_Amount(CompanyID, UserID, MarkupPercentage, 0, '', false, false, true);
    CostPrice = Eprint_ReturnFinal_Formated_Amount(CompanyID, UserID, CostPrice, 0, '', false, false, true);
    ...
    if (index == 0) {
        for (var i = 0; i < 4; i++) {
            ...
            if (MarkupPercentage != "" && !isNaN(MarkupPercentage)) {
                if (document.getElementById(spnPriceId) != null) {
                    //document.getElementById(txtmarkupId).value = MarkupPercentage;
                    document.getElementById(txtCostPrice).value = CostPrice;
                    var markup = MarkupPercentage;
                    //var markup = document.getElementById(txtmarkupId).innerHTML;;
                    //var price = document.getElementById(spnPriceId).innerHTML;
                    var price = CostPrice;
                    //var MarkupValue = (markup * price) / 100;   
                    //var SellPrice = Number(MarkupValue) + Number(price);
