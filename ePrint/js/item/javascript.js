...
/**
* @version		$Id: joomla.javascript.js 9108 2007-10-02 15:32:16Z mmaison $
* @package		Joomla
...
/* changes grid background color when checkbox clicked */

function changeGridColor(val, grid) {
    for (var i = 1; i < grid.rows.length; i++) {
        ...
        for (var j = 0; j < inputs.length; j++) {
            var changedcolor = "#FFFF99";
            if (inputs[j].checked == true) {
                grid.rows[i].style.backgroundColor = changedcolor;
            }
            else {
                grid.rows[i].style.backgroundColor = color;
            }
        }
    }
}

/* Changes background color when mouseover in grid view*/
function ItemMouseOver(oRow, chkBx) {
...
/* Changes background color when mouseover in grid view*/
function ItemMouseOut(oRow, chkBx) {
    oCheckBox = document.getElementById(chkBx);
    if (!oCheckBox.checked) {
        oCheckBox.style.display = 'block';
