...
function ShowWarehouseEdit(type, nu, txtVal) {
    if (type == "open") {
        ...
        document.getElementById("txtWareQty").focus();
        WareNu = nu;
    }
    else if (type == "close") {
...
function LoadEditedWare() {
    ...
    for (var p = 0; p < warearray.length; p++) {
        ...
        dd += "<div style='float:left;width:50px;'><a href='#' onclick=\"ShowWarehouseEdit('open'," + p + "," + warearray[p].Quantity + ");\">" + warearray[p].Quantity + "</a></div><div style='float:right;'>";
        //dd += "<a href='#' onclick='RemoveWare(" + p + ");' class='normaltext'>Remove</a></div></div>";
        dd += "<img style='cursor:pointer;float:right;height:10px;width:10px' src='" + strImagepath + "delete.gif' border='0' title='Remove' onclick=javascript:RemoveWare(" + p + "); />";
        dd += "</div></div>";
    }
...
function WarehouseAddButton(AddType) {
    debugger;
    if (!CheckDecimalPlus(txtWarehouseQuantity.value, 'spn_txtWarehouseQuantity', 'spn_txtWarehouseQuantity_number', 'yes')) { //CheckReqCompare(txtWarehouseQuantity.value, 'spn_txtWarehouseQuantity', 'spn_txtWarehouseQuantity_number')) {
...
//                    if (moduleTyp.toLowerCase() == "jobs" || moduleTyp.toLowerCase() == "invoice") {
//                        if (reduceStockTypeForCancelled.toString().toLowerCase() == "p" && (WareID1 != oldWareItemID)) {
//                            if (window.confirm('Do you want the quantity to be added back to the inventory?')) {
//                                hdnStockReduce.value = "Yes";
//                            }
//                        }
//                    }
//                }
...
function ShowAddPanel_Estimate(WID, WName, WType, UPrice, packedin, PackedInQty) {
    ...
    showDivPopupCenter('div_inventory_select', '200');
    // showDivPopupCenterWithoutBlur('div_inventory_select', '100');
    WareID1 = WID;
    WareName1 = WName;
    WareType1 = WType;
    UnitPrice1 = UPrice;
    PackedInQty1 = PackedInQty;
    txtWarehouseQuantity.value = Eprint_ReturnFinal_Formated_Amount(CompanyID, UserID, packedin, 0, "", true, false, true);
...
function WarehouseSelectedItem(WareID, WareName, WareType, Warevalue, WareItemID, PackedInQty) {
    ...
    objWare.Quantity = Eprint_ReturnFinal_Formated_Amount(CompanyID, UserID, txtWarehouseQuantity.value, 0, '', true, false, true); 

    if (typeof txtWarehouseQuantity2.value === "undefined") {
        objWare.Quantity2 = 0;
    }
    else {
        objWare.Quantity2 = Eprint_ReturnFinal_Formated_Amount(CompanyID, UserID, txtWarehouseQuantity2.value, 0, '', true, false, true); 
    }

    if (typeof txtWarehouseQuantity3.value === "undefined") {
        objWare.Quantity3 = 0;
    }
    else {
        objWare.Quantity3 = Eprint_ReturnFinal_Formated_Amount(CompanyID, UserID, txtWarehouseQuantity3.value, 0, '', true, false, true); 
    }

    if (typeof txtWarehouseQuantity4.value === "undefined") {
        objWare.Quantity4 = 0;
    }
    else {
        objWare.Quantity4 = Eprint_ReturnFinal_Formated_Amount(CompanyID, UserID, txtWarehouseQuantity4.value, 0, '', true, false, true); 
    }
...
function RemoveWare(nos) {
    ...
    if (estimateType == "warehouse") {
        if (warearray[nos].WareItemID != "0") {
            ...
            if (moduleTyp.toLowerCase() == "jobs" || moduleTyp.toLowerCase() == "invoice") {
                if (reduceStockTypeForCancelled == "p" && inventoryManagement == "IM") {
                    if (window.confirm('Do you want the quantity to be added back to the inventory?')) {
                        //hdnStockReduceNew.value = warearray[nos].ItemID + "±" + "Yes" + "±" + warearray[nos].Quantity + "µ";
                        // hdnStockReduce.value = warearray[nos].ItemID + "±" + "Yes" + "±" + warearray[nos].Quantity + "µ";
                        hdnStockReduce.value = "Yes";
                    }
                    // showDivAlert(warearray.length);
                }
            }
...
function SaveDataForWarehouse() {
    ...
    for (var p = 0; p < warearray.length; p++) {
        ...
        if (typeof warearray[p].Quantity2 === "undefined") {
            wareData += "Quantity2 »" + 0 + '±';
        }
        else {
            wareData += "Quantity2 »" + warearray[p].Quantity2 + '±';
        }
        if (typeof warearray[p].Quantity3 === "undefined") {
            wareData += "Quantity3 »" + 0 + '±';
        }
        else {
            wareData += "Quantity3 »" + warearray[p].Quantity3 + '±';
        }
        if (typeof warearray[p].Quantity4 === "undefined") {
            wareData += "Quantity4 »" + 0 + '±';
        }
        else {
            wareData += "Quantity4 »" + warearray[p].Quantity4 + '±';
        }
    ...
    if (wareData == '' || (ItmCnt == warearray.length)) {
        alert('Please select at least one Warehouse Item');
        return false;
    }
    else if (wareData != '') {
        hid_warehouse_save.value = wareData;
