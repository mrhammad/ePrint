...
{
    public partial class customize_header : BaseClass, IRequiresSessionState
    {
        ...
        static customize_header()
        {
            customize_header.appID = 0;
            customize_header.OrigFile_image = string.Empty;
            customize_header.NewFile_image = string.Empty;
        }
        ...
        protected void btn_Save_Click(object sender, EventArgs e)
        {
            ...
            if (this.fu_header.HasFile)
            {
                string[] strArrays = this.fu_header.FileName.ToString().Trim().Split(new char[] { '.' });
                ...
                if (str.ToLower() == "jpg" || str.ToLower() == "png" || str.ToLower() == "tif" || str.ToLower() == "jpeg" || str.ToLower() == "bmp" || str.ToLower() == "jpf" || str.ToLower() == "gif" || str.ToLower() == "msp" || str.ToLower() == "mng" || str.ToLower() == "pct" || str.ToLower() == "yuv" || str.ToLower() == "tiff" || str.ToLower() == "mng" || str.ToLower() == "jfif" || str.ToLower() == "dib" || str.ToLower() == "jpe")
                {
                    ...
                    customize_header.OrigFile_image = string.Concat(secureDocPath3);
                    ...
                    else
                    {
                        if (this.txt_width.Text != "")
                        {
                            ...
                            using (System.Drawing.Image image1 = System.Drawing.Image.FromFile(customize_header.OrigFile_image))
                            {
                                if (image1.Width <= num)
                                {
                                    this.saveImg = this.bannerImage;
                                ...
                                else
                                {
                                    try
                                    {
                                        System.Drawing.Image image2 = System.Drawing.Image.FromFile(customize_header.OrigFile_image);
                                        image = createImageThumnail.FixedSize(customize_header.OrigFile_image, image2, num, image1.Height, true, this.m_bannerImage);
                                        image2.Dispose();
                                        image.Dispose();
                                        GC.Collect();
                                    }
                    ...
                    this.objcreateImg.InsertThumbCommand(string.Concat(" /C imconvert ", customize_header.OrigFile_image, "[251x71]", this.m_bannerImage));
                }
                else
                {
                    this.lblError.Text = "Please upload only image file";
                    this.lblError.Style.Add("display", "block");
                    this.fu_header.Style.Add("display", "block");
            ...
            if (this.flag)
            {
                ...
                {
                    WebstoreBasePage.headerInsert(customize_header.appID, this.CompanyID, this.AccountID, "", base.SpecialEncode(this.txt_text.Text), "", "T", "H", false, false, 0);
                }
                if (this.rdimage.Checked)
                {
                    WebstoreBasePage.headerInsert(customize_header.appID, this.CompanyID, this.AccountID, this.saveImg, "", "", "I", "H", this.rdomakeitcentpercent.Checked, this.rdologosize.Checked, Convert.ToInt32(this.txt_width.Text));
                }
                if (this.rdtemplate.Checked)
                {
                    WebstoreBasePage.headerInsert(customize_header.appID, this.CompanyID, this.AccountID, "", "", base.SpecialEncode(this.txt_template.Text), "C", "H", false, false, 0);
                }
                base.Response.Redirect("customize_header.aspx?suc=up");
            }
        }

        protected void Page_Load(object sender, EventArgs e)
        {
            this.chkdefault.Text = this.objLanguage.GetLanguageConversion("Restore_Default");
            ...
            if (!base.IsPostBack)
            {
                foreach (DataRow row in WebstoreBasePage.headerSelect(this.CompanyID, this.AccountID, "H").Rows)
                {
                    customize_header.appID = Convert.ToInt32(row["appID"].ToString());
                    if (row["logoType"].ToString() == "I")
                    {
                        ...
                        this.fu_header.Style.Add("display", "none");
                    ...
                    else if (row["logoType"].ToString() != "T")
                    {
                        ...
                        this.txt_template.Text = base.SpecialDecode(row["logoTemplate"].ToString());
                    }
                    else
                    {
                        this.rdimage.Checked = false;
                        ...
                        this.rdtemplate.Checked = false;
                        this.txt_text.Text = base.SpecialDecode(row["logoText"].ToString());
                    }
                }
            ...
            base.Navigation_Path(string.Concat(languageConversion), string.Concat("&nbsp;>&nbsp;", this.objLanguage.GetLanguageConversion("Customize_Header")));
            global.pageName = "setting";
            global.pgName = "";
            this.gloobj.setpagename("setting");
            base.Title = this.objLanguage.convert(global.pageTitle("Customize Header", int.Parse(this.Session["companyid"].ToString()), this.Session["companyName"].ToString()));
            this.header.dtAccountList = this.objAcc.accountsList(this.CompanyID);
            this.header.SettingName = this.objLanguage.GetLanguageConversion("Customize_Header");
