...
{
    public partial class invoice_productcatalogue : BaseClass, IRequiresSessionState
    {
        ...
        public invoice_productcatalogue()
        {
        }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (base.Request.Params["type"] != null)
            {
                ...
                {
                    this.req_type = "add";
                    base.Navigation_Path(string.Concat("<a href=../invoice/invoice_view.aspx class='subnavigator'  style='text-decoration:underline;'>", this.objLanguage.GetLanguageConversion("Invoice_View"), "</a>"), string.Concat("&nbsp;>&nbsp;", this.objLanguage.GetLanguageConversion("Invoice_Add_Product_Catalogue")));
                ...
                else if (string.Compare(base.Request.Params["type"].ToString(), "edit", true) == 0)
                {
                    ...
                    {
                        object[] orderID = new object[] { "<a href='", this.strSitepath, "invoice/invoice_order_summary.aspx?frm=view&ordid=", this.OrderID, "&estid=", this.EstimateID, "&InvID=", this.InvoiceID, "' class='subnavigator'  style='text-decoration:underline;'>Invoice Summary</a>&nbsp;></a>" };
                        empty = string.Concat(orderID);
                    }
                    if (this.IsItemCompleted == 1)
                    {
                        base.Navigation_Path(string.Concat("<a href=", this.strSitepath, "invoice/invoice_view.aspx class='subnavigator'  style='text-decoration:underline;'>Invoice View</a> &nbsp;>&nbsp;", empty), "&nbsp;Invoice Edit: Product Catalogue");
                    ...
                    {
                        base.Navigation_Path("<a href=../jobs/jobs_view.aspx class='subnavigator'  style='text-decoration:underline;'>Invoice View</a>", string.Concat("&nbsp;>&nbsp;", empty, "Invoice Add: Product Catalogue"));
                        base.Title = this.objLanguage.convert(global.pageTitle("Invoice Add", int.Parse(this.Session["companyid"].ToString()), this.Session["companyName"].ToString()));
                    }
                }
            }
