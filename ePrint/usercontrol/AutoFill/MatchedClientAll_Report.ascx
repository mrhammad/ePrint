<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="MatchedClientAll_Report.ascx.cs" Inherits="ePrint.usercontrol.AutoFill.MatchedClientAll_Report" %>
    ...
    <div id="divText" style="float: left; border: solid 1px #868485; width: 270px; height: 150px;
        ...
        <div class="arrowlistmenu">
            <ul class="categoryitems">
                <li><a href="#" onclick="BindResultCustomer('1','Owned by Self','','','','','','','','','');">
                    Owned by Self </a></li>
            </ul>
            <ul>
                <li>&nbsp;&nbsp;Owned by Clients</li>
            </ul>
            <ul class="categoryitems">
                <asp:Repeater runat="server" ID="uc" OnItemDataBound="Repeater_OnItemDataBound">
                    ...
                    <asp:Label ID="hdnClientID" runat="server" Text='<%#Eval("ClientID")%>' Visible="false"></asp:Label>
                        ...
                        <li><a id="ClientName" runat="server" class="AutoFill">
                            <%#Eval("ClientName")%></a></li>   <%--onclick="BindResultCustomer('<%#Eval("ClientID")%>','<%#Eval("ClientName")%>','<%#Eval("Contacts")%>','<%#Eval("AccountNumber")%>','<%#Eval("ConcatDelAddress")%>','<%#Eval("DepartmentName")%>','<%#Eval("DepartmentID")%>', '<%#Eval("InvoiceAddressID")%>','<%#Eval("InvoiceAddress")%>','<%#Eval("ContactAddress")%>','<%#Eval("ContactAddressID")%>' );"--%>
                    </ItemTemplate>
                </asp:Repeater>
            </ul>
        </div>
