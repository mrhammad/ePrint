<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="PurchaseNo_ListShow.ascx.cs" Inherits="ePrint.usercontrol.StoreSettings.PurchaseNo_ListShow" %>
...
<div >  <%--class="borderwithouttop"--%>
                ...
                <ContentTemplate>
                    <telerik:RadGrid ID="RadGrid_Customer" runat="server" ShowHeader="true" Width="100%"
                        ...
                        <MasterTableView AllowFilteringByColumn="true" OverrideDataSourceControlSorting="true">
                            <Columns>
                                <telerik:GridTemplateColumn AllowFiltering="true" HeaderText="Purchase No." HeaderStyle-Width="45%"
                                    ItemStyle-Width="50%" CurrentFilterFunction="Contains" DataField="PONO"
                                    SortExpression="PONO" UniqueName="PONO" FilterControlWidth="18%" AutoPostBackOnFilter="true">
                                    <ItemTemplate>
                                        <div style="float: left; width: 98%; padding-left: 10px; max-height: 18px; height: 18px;">
                                            <asp:HyperLink ID="lblPurchaseNo" runat="server" Text='<%# Eval("PONO") %>' OnClick='<%# "javascript:lblPurchaseNo_Click(\"" + DataBinder.Eval(Container.DataItem, "PurchaseID") + "\");" %>'></asp:HyperLink>
                                        </div>
                                    </ItemTemplate>
 ...
 <script type="text/javascript" language="javascript">
     function lblPurchaseNo_Click(ID) {
         debugger

         window.open("<%=nmsCommon.global.sitePath()%>purchase/purchase_add.aspx?type=edit&id=" + ID, '_blank');
     }
    </script>
