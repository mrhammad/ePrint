...
namespace Printcenter.BusinessAccessLayer.Order
{
	public class Orders
	{
		public Orders()
		...
		public static void Insert_OrderAdditionalItems(long OrderID, long OrderItemID, long OptionID, string Formula, decimal Markup, decimal TotalPrice, decimal SelectedID, string SelectedValue, decimal SelectedPrice, decimal MarkupValue, int SortOrderNo, long ParentWebOtherCostID, string WebOtherCostType, string FreeTextQuestionLong = "")
		{
			...
			dbOrder.Insert_OrderAdditionalItems(OrderID, OrderItemID, OptionID, Formula, Markup, TotalPrice, SelectedID, SelectedValue, SelectedPrice, MarkupValue, SortOrderNo, ParentWebOtherCostID, WebOtherCostType,FreeTextQuestionLong);
		}

		public static DataTable JobInvoiceAllItemDetails_select(long ModuleID, string ModuleType)
		{
			return (new DbOrder()).JobInvoiceAllItemDetails_select(ModuleID, ModuleType);
		}

		public static DataTable jobOrder_select(int CompanyID, long OrderID, long EstimateID)
		{
			return (new DbOrder()).jobOrder_select(CompanyID, OrderID, EstimateID);
		}
		...
		{
			DbOrder dbOrder = new DbOrder();
			return dbOrder.order_advance_search(CompanyID, SearchText, ClientID, Status, DateType, FromDate, ToDate, IsArchived, TodayDate, StartDate, EndDate, SalesPersonRecords);
		}
		...
		{
			DbOrder dbOrder = new DbOrder();
			return dbOrder.order_advance_search_SplitItems(CompanyID, SearchText, ClientID, Status, DateType, FromDate, ToDate, IsArchived, TodayDate, StartDate, EndDate, SalesPersonRecords);
		}

		public static DataTable Order_AllItemsDetails_Select(long ModuleID, string ModuleType)
		{
			return (new DbOrder()).Order_AllItemsDetails_Select(ModuleID, ModuleType);
		}

		public static DataTable Order_select(int CompanyID, long OrderID)
		{
			return (new DbOrder()).Order_select(CompanyID, OrderID);
		}

		public static DataTable Order_select_Summary_PerItem(int CompanyID, long EstimateItemID)
		{
			return (new DbOrder()).Order_select_Summary_PerItem(CompanyID, EstimateItemID);
		}
		...
		{
			DbOrder dbOrder = new DbOrder();
			dbOrder.OrderAdditionalOptions_Update(OrderID, Formula, Markup, TotalPrice, SelectedID, SelectedValue, MarkupValue, SortOrderNo, SelectedPrice, TaxID, TaxPercentage, TotalTaxValue);
		}

		public static string OrderConvertedToJob(long OrderID)
		{
			return (new DbOrder()).OrderConvertedToJob(OrderID);
		}
		...
		{
			DbOrder dbOrder = new DbOrder();
			dbOrder.Update_OrdersItems(CompanyID, OrderID, OrderItemID, ProductID, JobName, Quantity, UnitPrice, UploadFile, UploadFile1, UploadFile2, MainItemPrice, TaxRate, MainItemMarkupPrice, OrderItemTotalPrice, OrderItemTax, OrderItemShipping, ItemDesc, TotalMarkupPrice, CampaignID, Height, Width, Markup, TaxId);
		}
	}
}