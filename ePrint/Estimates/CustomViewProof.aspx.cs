...
{
	public partial class CustomViewProof : System.Web.UI.Page
	{
        ...
        protected void btndelete_OnClick(object sender, EventArgs e)
        {
            ...
            CompanyBasePage.View_Delete(this.companyId, "proof", this.delViewID, this.UserID, "proof_view");
            ...
            {
                string str = string.Concat("Proofs", this.UserID, "Proofs");
                base.Session["search_proof"] = null;
                base.Session[str] = null;
                base.Session["ProofViewID"] = null;
            }
            base.Response.Redirect(string.Concat(this.strSitepath, "estimates/CustomViewProof.aspx?type=del"));
        }

        protected void btnMove_OnClick(object sender, EventArgs e)
        {
            CustomViewProof.MoveRecords(this.lstClumns, this.lstSelectedCols);
        }

        protected void btnReMove_OnClick(object sender, EventArgs e)
        {
            CustomViewProof.ReMoveRecords(this.lstSelectedCols, this.lstClumns);
        }
        ...
        protected void btnsave_OnClick(object sender, EventArgs e)
        {
            ...
            {
                CustomViewProof usercontrolViewsCustomView = this;
            ...
            if (base.Request.Params["type"] == null || base.Request.Params["id"] == null)
            {
                ...
                if (this.isdefault == 1)
                {
                    this.comm.UserSetting_Update(this.companyId, this.UserID, "proof_view", this.ret.ToString());
                }
            }
            else
            {
            ...
            this.objCreateView.View_Set_Default_All_zero_exist("proof", this.companyId);
            if (this.ret <= 0)
            {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            base.Title = this.objLanguage.convert(global.pageTitle("Proof View", int.Parse(this.Session["companyid"].ToString()), this.Session["companyName"].ToString()));
            ...
            if (!base.IsPostBack)
            {
                ...
                for (int i = 0; i < this.lstClumns.Items.Count; i++)
                {
                    ...
                    if (this.lstClumns.Items[i].Value.ToLower() == "proofnumber")
                    {
                        this.lstClumns.Items[i].Text = "Proof No.";
                    }
                    if (this.lstClumns.Items[i].Value.ToLower() == "itemtitle")
                    {
                        this.lstClumns.Items[i].Text = "Item Title";
                    }
                    if (this.lstClumns.Items[i].Value.ToLower() == "jobtitle")
                    {
                        this.lstClumns.Items[i].Text = "Job Title";
                    }
                    if (this.lstClumns.Items[i].Value.ToLower() == "proofstatus")
                    {
                        this.lstClumns.Items[i].Text = "Proof Status";
                    }
                    if (this.lstClumns.Items[i].Value.ToLower() == "proofdate")
                    {
                        this.lstClumns.Items[i].Text = "Proof Date";
                    }
                    if (this.lstClumns.Items[i].Value.ToLower() == "filename")
                    {
                        this.lstClumns.Items[i].Text = "File Name";
                    }
            ...
            if (base.Request.Params["type"] != null)
            {
                if (base.Request.Params["type"].ToString().ToLower() == "edit" && base.Request.Params["id"] != null)
                {
                    ...
                    this.ViewID = Convert.ToInt64(base.Request.Params["id"].ToString());
                    this.iszero = this.objCreateView.Check_Iszeroth_View(this.companyId, this.ViewID, "proof");
                    ...
                    if (!base.IsPostBack)
                    {
                        DataTable dataTable = EstimateBasePage.Estimates_ViewName_Select_By_ID(BaseClass.CheckIntegerNull(this.companyId), this.ViewID, "proof");
