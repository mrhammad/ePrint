...
BusyBox.prototype.RenderContent = function()
{
	...
	var style = " style='background-color:#990033;BORDER:0px; POSITION: absolute;' ";
	
	doc.open();
	doc.writeln("<body ondragstart='return false;' style='Margin: 0px; Background-Color: #990033'>");
	doc.writeln("   <div id='" + this.DivID + "' align=center " + style + ">");
//	doc.writeln("      <img id='" + this.ImgID + "' src=''>");
	doc.writeln("      <font color=#FFFFFF><b>&nbsp;Loading...</b></font>");
	doc.writeln("   </div>");
	doc.writeln("</body>");
	doc.close();
}
...
BusyBox.prototype.Resize = function()
{
	...
	if( BusyBox.IsBrowserIE() )
	{
		...
		this.IFrame.style.height = 20;//div.offsetHeight;
	}
	else
	{
		// Set the width to the value specified.
		this.IFrame.style.width = this.Width;
		this.IFrame.style.height = 20;//this.Height;
	}
}
...
BusyBox.prototype.Center = function()
{
	...
	//rajeev
	objLeft = document.body.clientWidth - this.IFrame.offsetWidth;
	objTop = 0;//this.IFrame.offsetHeight;
	objLeft = objLeft + document.body.scrollLeft;
	objTop = objTop + document.body.scrollTop;
