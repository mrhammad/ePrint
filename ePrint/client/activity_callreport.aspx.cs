...
{
    public partial class activity_callreport : BaseClass, IRequiresSessionState
    {
        ...
        public string flag_Duration = string.Empty;

        public int cellvalue_Duration;
        ...
        public void btnExport_New_OnClick(object sender, EventArgs e)
        {
            ...
            DataSet callDetailsData = this.GetCallDetailsData(0);
            DataTable item = callDetailsData.Tables[0];
            if (item != null)
            {
                string empty = string.Empty;
                foreach (DataRow row in item.Rows)
                {
                    row.Table.Columns.Contains("DateAndTime");
                }
            }
            this.GetCallDetailsData(0);
            DataTable languageConversion = callDetailsData.Tables[0];
            ...
            foreach (DataColumn column in languageConversion.Columns)
            {
                for (int j = 0; j < languageConversion.Columns.Count; j++)
                {
                    ...
                    if (languageConversion.Columns[j].ColumnName == "ContactNumber")
                    {
                        languageConversion.Columns[j].ColumnName = this.objLangClass.GetLanguageConversion("Contact_Number");
                    }
                    ...
                    if (languageConversion.Columns[j].ColumnName == "CallType")
                    {
                        languageConversion.Columns[j].ColumnName = this.objLangClass.GetLanguageConversion("Call_Type");
                    }
                    if (languageConversion.Columns[j].ColumnName == "CallDetails")
                    {
                        languageConversion.Columns[j].ColumnName = this.objLangClass.GetLanguageConversion("Call_Details");
                    }
                    if (languageConversion.Columns[j].ColumnName == "CallPurpose")
                    {
                        languageConversion.Columns[j].ColumnName = this.objLangClass.GetLanguageConversion("Call_Purpose");
                    }
                    ...
                    {
                        languageConversion.Columns[j].ColumnName = this.objLangClass.GetLanguageConversion("Call_Start_date");
                    }
                }
                using (XLWorkbook wb = new XLWorkbook())
                {

                    wb.Worksheets.Add(languageConversion);

                    Response.Clear();
                    Response.Buffer = true;
                    Response.Charset = "";
                    Response.ContentType = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet";
                    Response.AddHeader("content-disposition", "attachment;filename=ActivityCall_Excel.xlsx");
        ...
        private DataTable SetCallReportColumns(DataSet callData)
        {
            DataSet callDetailsData = callData;
            DataTable item = callDetailsData.Tables[0];
            if (item != null)
            {
                string empty = string.Empty;
        ...
        public void btnExport_vivid_OnClick(object sender, EventArgs e)
        {
            ...
            string str = "ActivityCall_Presentation.xls";
        ...
        public void btnRunReport_OnClick(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    DataSet callDetailsData = this.GetCallDetailsData(1);
                    this.lblTotalRecords.Text = callDetailsData.Tables[1].Rows[0][0].ToString();
                    if (callDetailsData.Tables[0].Rows.Count == 0)
                    {
                    ...
                    else if (Convert.ToInt32(callDetailsData.Tables[1].Rows[0][0].ToString()) > 100)
                    {
                        this.GridEstReport.Visible = true;
                        this.div_Total.Visible = false;
                        this.pnlEmptyRecords.Visible = false;
                        //this.GridEstReport.DataSource = callDetailsData;
                        DataTable dt = SetCallReportColumns(callDetailsData);
                        this.GridEstReport.DataSource = dt;
                        //this.GridEstReport.RowDataBound += new GridViewRowEventHandler(this.GridActivityCallReport_RowDataBound);
                        this.GridEstReport.DataBind();
                        this.usrPaging.Visible = false;
                        pagingreport.intCurrentPage = this.PageNumber;
                        pagingreport.intPageCount = Convert.ToInt32(Math.Ceiling(Convert.ToDouble(callDetailsData.Tables[1].Rows[0][0].ToString()) / (double)this.PageSize));
        ...
        public void btnUpdateExisting_OnClick(object sender, EventArgs e)
        {
            ...
            if (this.hdn_reportID.Value == "")
            {
                ...
                sqlCommand.Parameters.AddWithValue("@Pagename", "activitycallreport");
        ...
        private DataSet GetCallDetailsData(int PageNumber)
        {
            string[] strArrays;
            object[] companyID;
            string str;
            ...
            this.txtFreeText.Text.ToString().Split(new char[] { ',' });
            if (this.txtFreeText.Text != "" || this.txtFreeText.Text != null)
            {
                empty10 = base.SpecialEncode(this.txtFreeText.Text);
                if (empty10.Contains(" "))
                {
                    strArrays = new string[] { " " };
                    string[] strArrays1 = empty10.Split(strArrays, StringSplitOptions.None);
                    empty10 = strArrays1[0].ToString();
            ...
            for (int i = 0; i < this.chkColumns.Items.Count; i++)
            {
                if (this.chkColumns.Items[i].Selected)
                {
                    foreach (ListItem item in this.chkColumns.Items)
                    {
                        ...
                        {
                            empty1 = "a.clientID,ltrim(a.clientName) as [CompanyName]";
                        }
                        if (item.Value == "ContactName")
                        {
                            empty1 = (empty1 == "" ? "ISNULL(RTRIM(LTRIM(isnull(b.firstname,'')+isnull(NULLIF(' '+b.middlename,' '),'')+isnull(' '+b.lastname,''))),'') as ContactName" : string.Concat(empty1, ",ISNULL(RTRIM(LTRIM(isnull(b.firstname,'')+isnull(NULLIF(' '+b.middlename,' '),'')+isnull(' '+b.lastname,''))),'') as ContactName"));
                        }
                        if (item.Value == "ContactNumber")
                        {
                            empty1 = (empty1 == "" ? "ISNULL((SELECT isnull(b.mobile,'') FROM tb_contact b WHERE b.contactId = c.ContactID and b.isDelete=0), '') as ContactNumber" : string.Concat(empty1, ",ISNULL((SELECT isnull(b.mobile,'') FROM tb_contact b WHERE b.contactId = c.ContactID and b.isDelete=0), '') as ContactNumber"));
                        }
                        ...
                        {
                            empty1 = (empty1 == "" ? "isnull(c.Subject,'') as Subject" : string.Concat(empty1, ",isnull(c.Subject,'') as Subject "));
                        }
                        if (item.Value == "CallType")
                        {
                            empty1 = (empty1 == "" ? "c.CallType as CallType" : string.Concat(empty1, ",c.CallType as CallType"));
                        }
                        if (item.Value == "CallDetails")
                        {
                            empty1 = (empty1 == "" ? " case when c.Calldetails='scheduled' then 'Scheduled' when c.Calldetails='completed'  then 'Completed' end as CallDetails " : string.Concat(empty1, ", case when c.Calldetails='scheduled' then 'Scheduled' when c.Calldetails='completed'  then 'Completed' end as CallDetails "));
                        }
                        if (item.Value == "CallPurpose")
                        {
                            empty1 = (empty1 == "" ? "isnull(d.StatusTitle,'') as CallPurpose" : string.Concat(empty1, ",isnull(d.StatusTitle,'') as CallPurpose"));
                        }
                        if (item.Value == "CallResult")
                        {
                            empty1 = (empty1 == "" ? "isnull(c.CallResult,'') as CallResult" : string.Concat(empty1, ",isnull(c.CallResult,'') as CallResult"));
                        }
                        if (item.Value == "AssignedTo")
                        {
                            if (empty1 == "")
                            {
                                empty1 = string.Concat("(Select isnull(u.firstname,'') from tb_user u  where u.CompanyID=", this.CompanyID, " and u.userid=c.AssignedTo and u.IsDelete=0)  as AssignedTo");
                            }
                            else
                            {
                                object[] objArray = new object[] { empty1, ",(Select isnull(u.firstname,'') from tb_user u  where u.CompanyID=", this.CompanyID, " and u.userid=c.AssignedTo and u.IsDelete=0)  as AssignedTo" };
                                empty1 = string.Concat(objArray);
                            }
                        }
                        if (item.Value == "Notes")
                        {
                            empty1 = (empty1 == "" ? "isnull(c.Description,'') as Description" : string.Concat(empty1, ",isnull(c.Description,'') as Description"));
                        }
                        if (item.Value != "DateAndTime")
                        {
                            continue;
                        }
                        empty1 = (empty1 == "" ? "isnull(c.CallStartdate,'') as DateAndTime" : string.Concat(empty1, ",isnull(c.CallStartdate,'') as DateAndTime"));
                    }
                }
                ...
                str1 = string.Concat(" ", str1, "left join tb_contact b on b.clientid=a.clientid and b.isDelete=0  ");
                str1 = string.Concat(" ", str1, "left join [tb_Customer_Call] c on c.clientid=a.clientid and c.isDeleted=0  AND c.ContactID = b.contactId    ");
                str1 = string.Concat(" ", str1, "left join tb_CrmCallPurpose d on d.id=c.callpurpose and d.IsDeleted=0  ");
                ...
                if (this.chkDateOption.Checked)
                {
                    ...
                    {
                        string str12 = this.objJava.eprint_checkdateformat_returnonlymmddyyyy(this.DateFormat, this.day[0].ToString());
                    ...
                    else if (this.rdlDate.SelectedValue == ePrintUtilities.ePrintConstants.ThisAnnualYear)
                    {
                    ...
                    {
                        string str13 = this.objJava.eprint_checkdateformat_returnonlymmddyyyy(this.DateFormat, base.ReplaceSingleQuote(this.txtFrom.Text));
                        string str14 = this.objJava.eprint_checkdateformat_returnonlymmddyyyy(this.DateFormat, base.ReplaceSingleQuote(this.txtTo.Text));
                        string[] strArrays3 = new string[] { " ", str1, " and DateAdd(d,0,DateDiff(d,0,c.CallStartdate)) BETWEEN '", str13, "' AND '", str14, "' " };
                        str1 = string.Concat(strArrays3);
                    }
                }
                if (this.ddlCalls.SelectedValue != "0")
                {
                    if (this.ddlCalls.SelectedValue == "Completed")
                    {
                        str1 = string.Concat(" ", str1, " and c.Calldetails='Completed' ");
                    }
                    else if (this.ddlCalls.SelectedValue == "OverDue")
                    {
                        str1 = string.Concat(" ", str1, " and c.CallStartdate <= CONVERT(nvarchar(20), GETDATE(),101) and c.Calldetails!='Completed' ");
                    }
                }
                if (this.txtFreeText.Text != "")
                {
                    ...
                    for (int k = 0; k < this.chkFreeText.Items.Count; k++)
                    {
                        if (num > 1)
                        {
                            if (this.chkFreeText.Items[k].Selected)
                            {
                                ...
                                if (this.chkFreeText.Items[k].Value == "ContactName")
                                {
                                    ...
                                    str1 = string.Concat(str1, " (RTRIM(LTRIM(isnull(b.firstname,'')+isnull(NULLIF(' '+b.middlename,' '),'')+isnull(' '+b.lastname,''))) like '%", base.SpecialEncode(this.txtFreeText.Text.Trim()), "%')");
                                }
                                ...
                                {
                                    str1 = (num1 != 0 ? string.Concat(str1, " or ") : string.Concat(str1, " and ( "));
                                    object[] companyID2 = new object[] { str1, " ((Select isnull(u.firstname,'') from tb_user u  where u.CompanyID=", this.CompanyID, " and u.userid=c.AssignedTo and u.IsDelete=0) like '%", base.SpecialEncode(this.txtFreeText.Text.Trim()), "%')  " };
                                    str1 = string.Concat(companyID2);
                                }
                                if (this.chkFreeText.Items[k].Value == "Subject")
                                {
                                    str1 = (num1 != 0 ? string.Concat(str1, " or ") : string.Concat(str1, " and ( "));
                                    str1 = string.Concat(str1, " (isnull(c.Subject,'') like '%", base.SpecialEncode(this.txtFreeText.Text.Trim()), "%') ");
                                }
                                if (this.chkFreeText.Items[k].Value == "Description")
                                {
                                    str1 = (num1 != 0 ? string.Concat(str1, " or ") : string.Concat(str1, " and ( "));
                                    str1 = string.Concat(str1, " (isnull(c.Description,'') like '%", base.SpecialEncode(this.txtFreeText.Text.Trim()), "%') ");
                                }
                                num1++;
                            }
                        }
                        else if (num == 1 && this.chkFreeText.Items[k].Selected)
                        {
                            if (this.chkFreeText.Items[k].Value == "CompanyName")
                            {
                                str1 = string.Concat(str1, " and ( (ltrim(a.clientName) like '%", base.SpecialEncode(this.txtFreeText.Text), "%')");
                            }
                            if (this.chkFreeText.Items[k].Value == "ContactName")
                            {
                                str1 = string.Concat(str1, " and (RTRIM(LTRIM(isnull(b.firstname,'')+isnull(NULLIF(' '+b.middlename,' '),'')+isnull(' '+b.lastname,''))) like '%", base.SpecialEncode(this.txtFreeText.Text.Trim()), "%'");
                            }
                            if (this.chkFreeText.Items[k].Value == "SalesPerson")
                            {
                            ...
                            if (this.chkFreeText.Items[k].Value == "AssignedTo")
                            {
                                object[] companyID3 = new object[] { str1, " and ( ((Select isnull(u.firstname,'') from tb_user u  where u.CompanyID=", this.CompanyID, " and u.userid=c.AssignedTo and u.IsDelete=0) like '%", base.SpecialEncode(this.txtFreeText.Text.Trim()), "%')" };
                                str1 = string.Concat(companyID3);
                            }
                            if (this.chkFreeText.Items[k].Value == "Subject")
                            {
                                str1 = string.Concat(str1, " and ( (isnull(c.Subject,'') like '%", base.SpecialEncode(this.txtFreeText.Text.Trim()), "%') ");
                            }
                            if (this.chkFreeText.Items[k].Value == "Description")
                            {
                                str1 = string.Concat(str1, " and ( (isnull(c.Description,'') like '%", base.SpecialEncode(this.txtFreeText.Text.Trim()), "%')");
                            }
                        }
                    }
                    if (num != 0)
            ...
            if (PageNumber != 0)
            {
                empty13 = "CREATE TABLE #PageIndexNew( IndexId INT IDENTITY (1, 1) NOT NULL";
                if (this.chkColumns.Items[0].Selected)
                {
                    empty13 = (empty13 == "" ? "clientID int" : string.Concat(empty13, " ,clientID int"));
                    empty13 = (empty13 == "" ? "CompanyName nvarchar(max)" : string.Concat(empty13, " ,CompanyName nvarchar(max)"));
                }
                if (this.chkColumns.Items[1].Selected)
                {
                    empty13 = (empty13 == "" ? "ContactName nvarchar(max)" : string.Concat(empty13, " ,ContactName nvarchar(max)"));
                }
                if (this.chkColumns.Items[2].Selected)
                {
                    empty13 = (empty13 == "" ? "ContactNumber nvarchar(50)" : string.Concat(empty13, " ,ContactNumber nvarchar(50)"));
                }
                if (this.chkColumns.Items[3].Selected)
                {
                    empty13 = (empty13 == "" ? "SalesPerson nvarchar(max)" : string.Concat(empty13, " ,SalesPerson nvarchar(max)"));
                }
                if (this.chkColumns.Items[4].Selected)
                {
                    empty13 = (empty13 == "" ? "Subject nvarchar(max)" : string.Concat(empty13, " ,Subject nvarchar(max)"));
                }
                if (this.chkColumns.Items[5].Selected)
                {
                    empty13 = (empty13 == "" ? "CallType nvarchar(500)" : string.Concat(empty13, " ,CallType nvarchar(500)"));
                }
                if (this.chkColumns.Items[6].Selected)
                {
                    empty13 = (empty13 == "" ? "CallDetails nvarchar(max)" : string.Concat(empty13, " ,CallDetails nvarchar(max)"));
                }
                if (this.chkColumns.Items[7].Selected)
                {
                    empty13 = (empty13 == "" ? "CallPurpose nvarchar(max)" : string.Concat(empty13, " ,CallPurpose nvarchar(max)"));
                }
                if (this.chkColumns.Items[8].Selected)
                {
                    empty13 = (empty13 == "" ? "AssignedTo nvarchar(max)" : string.Concat(empty13, " ,AssignedTo nvarchar(max)"));
                }
                if (this.chkColumns.Items[9].Selected)
                {
                    empty13 = (empty13 == "" ? "Description nvarchar(max)" : string.Concat(empty13, " ,Description nvarchar(max)"));
                }
                if (this.chkColumns.Items[10].Selected)
                {
                    empty13 = (empty13 == "" ? "DateAndTime nvarchar(250)" : string.Concat(empty13, " ,DateAndTime  nvarchar(250)"));
                }
                empty13 = string.Concat(empty13, ")");
                empty13 = string.Concat(empty13, "Insert Into #PageIndexNew  (");
                if (this.chkColumns.Items[0].Selected)
                {
                    empty13 = (empty13 == "" ? "clientID" : string.Concat(empty13, " clientID"));
                    empty13 = (empty13 == "" ? "CompanyName" : string.Concat(empty13, " ,CompanyName "));
                }
                if (this.chkColumns.Items[1].Selected)
                {
                    empty13 = (empty13 == "" ? "ContactName" : string.Concat(empty13, " ,ContactName"));
                }
                if (this.chkColumns.Items[2].Selected)
                {
                    empty13 = (empty13 == "" ? "ContactNumber" : string.Concat(empty13, " ,ContactNumber"));
                }
                if (this.chkColumns.Items[3].Selected)
                {
                    empty13 = (empty13 == "" ? "SalesPerson" : string.Concat(empty13, " ,SalesPerson"));
                }
                if (this.chkColumns.Items[4].Selected)
                {
                    empty13 = (empty13 == "" ? "Subject" : string.Concat(empty13, " ,Subject"));
                }
                if (this.chkColumns.Items[5].Selected)
                {
                    empty13 = (empty13 == "" ? "CallType" : string.Concat(empty13, " ,CallType"));
                }
                if (this.chkColumns.Items[6].Selected)
                {
                    empty13 = (empty13 == "" ? "CallDetails" : string.Concat(empty13, " ,CallDetails"));
                }
                if (this.chkColumns.Items[7].Selected)
                {
                    empty13 = (empty13 == "" ? "CallPurpose" : string.Concat(empty13, " ,CallPurpose"));
                }
                if (this.chkColumns.Items[8].Selected)
                {
                    empty13 = (empty13 == "" ? "AssignedTo" : string.Concat(empty13, " ,AssignedTo"));
                }
                if (this.chkColumns.Items[9].Selected)
                {
                    empty13 = (empty13 == "" ? "Description" : string.Concat(empty13, " ,Description"));
                }
                if (this.chkColumns.Items[10].Selected)
                {
                    empty13 = (empty13 == "" ? "DateAndTime" : string.Concat(empty13, " ,DateAndTime"));
                }
                empty13 = string.Concat(empty13, ")");
                if (this.chkColumns.Items[4].Selected || this.chkColumns.Items[5].Selected || this.chkColumns.Items[6].Selected || this.chkColumns.Items[7].Selected || this.chkColumns.Items[8].Selected || this.chkColumns.Items[9].Selected || this.chkColumns.Items[10].Selected)
                {
                    string str15 = empty13;
                    string[] strArrays4 = new string[] { str15, "select  ", empty1, " ", str1, "AND c.isDeleted=0 order by [CompanyName] " };
                    empty13 = string.Concat(strArrays4);
                }
                else
                {
                    str = empty13;
                    strArrays = new string[] { str, "select distinct ", empty1, " ", str1, "AND c.isDeleted=0 order by [CompanyName] " };
                    empty13 = string.Concat(strArrays);
                }
                int pageNumber = 0;
                int pageSize = 0;
                if (PageNumber != 1)
                ...
                empty13 = string.Concat(empty13, "select  ");
                if (this.chkColumns.Items[0].Selected)
                {
                    empty13 = (empty13 == "" ? "clientID" : string.Concat(empty13, " clientID"));
                    empty13 = (empty13 == "" ? "CompanyName" : string.Concat(empty13, " ,CompanyName"));
                ...
                empty13 = string.Concat(companyID);
            }
            else if (this.chkColumns.Items[4].Selected || this.chkColumns.Items[5].Selected || this.chkColumns.Items[6].Selected || this.chkColumns.Items[7].Selected || this.chkColumns.Items[8].Selected || this.chkColumns.Items[9].Selected || this.chkColumns.Items[10].Selected)
            {
                str = empty13;
                string[] strArrays5 = new string[] { str, "select  ", empty1, " ", str1, "AND c.isDeleted=0 order by [CompanyName] " };
                empty13 = string.Concat(strArrays5);
            }
            else
            {
                string str16 = empty13;
                string[] strArrays6 = new string[] { str16, "select distinct ", empty1, " ", str1, "AND c.isDeleted=0 order by [CompanyName] " };
                empty13 = string.Concat(strArrays6);
            }
            (new SqlDataAdapter(empty13, sqlConnection)).Fill(dataSet);
            if (this.ddlGrouprecords.SelectedValue.ToString().Trim() == "1")
            {
                this.btnExport.Visible = false;
                this.btnExportPPT.Visible = true;
                DataSet dataSet1 = new DataSet();
                string str17 = string.Concat("select distinct isnull(DATEADD(d, 0, DATEDIFF(d, 0, c.CreatedOn)),'') as CreatedDate ", str1, " order by CreatedDate");
                (new SqlDataAdapter(str17, sqlConnection)).Fill(dataSet1);
                foreach (DataRow row in dataSet1.Tables[0].Rows)
                {
                    ...
                    DateTime dateTime = Convert.ToDateTime(row["CreatedDate"]);
                    ...
                    if (label.Text != "")
                    {
                        ...
                        if (this.chkColumns.Items[5].Selected)
                        {
                            this.plhdetails.Controls.Add(new LiteralControl(string.Concat("<td class='headerstylereport1' align='left' width='5%' valign='top' style='vertical-align:middle;'><div style='min-width: 100px; max-height: 15px; height: 25px;float: left; width: 99%; overflow: hidden'><b>", this.objLangClass.GetLanguageConversion("Call_Type"), "</b>&nbsp;</div></td>")));
                        }
                        if (this.chkColumns.Items[6].Selected)
                        {
                            this.plhdetails.Controls.Add(new LiteralControl(string.Concat("<td class='headerstylereport1' align='left' width='5%' valign='top' style='vertical-align:middle;'><div style='min-width: 100px; max-height: 15px; height: 25px;float: left; width: 99%; overflow: hidden'><b>", this.objLangClass.GetLanguageConversion("Call_Details"), "</b>&nbsp;</div></td>")));
                        }
                        if (this.chkColumns.Items[7].Selected)
                        {
                            this.plhdetails.Controls.Add(new LiteralControl(string.Concat("<td class='headerstylereport1' align='left' width='5%' valign='top' style='vertical-align:middle;'><div style='min-width: 100px; max-height: 15px; height: 25px;float: left; width: 99%; overflow: hidden'><b>", this.objLangClass.GetLanguageConversion("Call_Purpose"), "</b>&nbsp;</div></td>")));
                        ...
                        if (this.chkColumns.Items[10].Selected)
                        {
                            this.plhdetails.Controls.Add(new LiteralControl(string.Concat("<td class='headerstylereport1' align='left' width='5%' valign='top' style='vertical-align:middle;'><div style='min-width: 100px; max-height: 15px; height: 25px;float: left; width: 99%; overflow: hidden'><b>", this.objLangClass.GetLanguageConversion("Call_Start_date"), "</b>&nbsp;</div></td>")));
                        }
                        this.plhdetails.Controls.Add(new LiteralControl("</tr>"));
                        string empty14 = string.Empty;
                        commonClass _commonClass1 = this.objJava;
                        dateTime = Convert.ToDateTime(row["CreatedDate"]);
                        string str18 = _commonClass1.Eprint_return_Date_Before_View(dateTime.ToShortDateString(), this.CompanyID, this.UserID, false);
                        str = empty14;
                        strArrays = new string[] { str, "select ", empty1, " ", str1, " and DATEADD(d, 0, DATEDIFF(d, 0, c.CreatedOn)) = '", this.objJava.eprint_checkdateformat_returnonlymmddyyyy(this.DateFormat, str18), "'" };
                        ...
                        while (sqlDataReader.Read())
                        {
                            ...
                            if (this.chkColumns.Items[2].Selected)
                            {
                                this.plhdetails.Controls.Add(new LiteralControl(string.Concat("<td class='table-rowrpt' align='left' width='5%' style='vertical-align:middle'><div id='division' style='overflow:auto;word-break:break-all;height:25px;min-width: 200px; max-height: 15px;float: left; width: 99%;padding-top:0px;' class='divscroll'>", this.objBase.SpecialDecode(sqlDataReader["ContactNumber"].ToString()), "&nbsp;</div></td>")));
                            }
                            if (this.chkColumns.Items[3].Selected)
                            {
                            ...
                            if (this.chkColumns.Items[5].Selected)
                            {
                                this.plhdetails.Controls.Add(new LiteralControl(string.Concat("<td class='table-rowrpt' align='left' width='5%' style='vertical-align:middle'><div id='division' style='overflow:auto;word-break:break-all;height:25px;min-width: 200px; max-height: 15px;float: left; width: 99%;padding-top:0px;' class='divscroll'>", this.objBase.SpecialDecode(sqlDataReader["CallType"].ToString()), "&nbsp;</div></td>")));
                            }
                            if (this.chkColumns.Items[6].Selected)
                            {
                                this.plhdetails.Controls.Add(new LiteralControl(string.Concat("<td class='table-rowrpt' align='left' width='5%' style='vertical-align:middle'><div id='division' style='overflow:auto;word-break:break-all;height:25px;min-width: 200px; max-height: 15px;float: left; width: 99%;padding-top:0px;' class='divscroll'>", this.objBase.SpecialDecode(sqlDataReader["CallDetails"].ToString()), "&nbsp;</div></td>")));
                            }
                            if (this.chkColumns.Items[7].Selected)
                            {
                                this.plhdetails.Controls.Add(new LiteralControl(string.Concat("<td class='table-rowrpt' align='left' width='5%' style='vertical-align:middle'><div id='division' style='overflow:auto;word-break:break-all;height:25px;min-width: 200px; max-height: 15px;float: left; width: 99%;padding-top:0px;' class='divscroll'>", this.objBase.SpecialDecode(sqlDataReader["CallPurpose"].ToString()), "&nbsp;</div></td>")));
                            }
                            if (this.chkColumns.Items[8].Selected)
                            {
                            ...
                            this.plhdetails.Controls.Add(new LiteralControl(string.Concat("<td class='table-rowrpt' align='left' width='5%' style='vertical-align:middle'><div id='division' style='overflow:auto;word-break:break-all;height:25px;min-width: 200px; max-height: 15px;float: left; width: 99%;padding-top:0px;' class='divscroll'>", this.objJava.Eprint_return_Date_Before_View(sqlDataReader["DateAndTime"].ToString(), this.CompanyID, this.UserID, false), "&nbsp;</div></td>")));
                        }
                        sqlConnection.Close();
            ...
            else if (this.ddlGrouprecords.SelectedValue.ToString().Trim() == "2")
            {
                ...
                companyID = new object[] { "select distinct (Select isnull(u.firstname,'') from tb_user u  where u.CompanyID=", this.CompanyID, " and u.userid=c.AssignedTo and u.IsDelete=0) as AssignedTo ", str1, " " };
                string str25 = string.Concat(companyID);
                (new SqlDataAdapter(str25, sqlConnection)).Fill(dataSet2);
                foreach (DataRow dataRow in dataSet2.Tables[0].Rows)
                {
                    ...
                    string str26 = this.objBase.SpecialDecode(dataRow["AssignedTo"].ToString());
                    ...
                    this.plhdetails.Controls.Add(new LiteralControl("</tr>"));
                    empty = string.Empty;
                    companyID = new object[] { empty, "select ", empty1, " ", str1, " and (Select isnull(u.firstname,'') from tb_user u  where u.CompanyID=", this.CompanyID, " and u.userid=c.AssignedTo and u.IsDelete=0) = '", dataRow["AssignedTo"], "' " };
                    ...
                    while (sqlDataReader1.Read())
                    {
                        ...
                        if (this.chkColumns.Items[2].Selected)
                        {
                            this.plhdetails.Controls.Add(new LiteralControl(string.Concat("<td class='table-rowrpt' align='left' width='5%' style='vertical-align:middle'><div id='division' style='overflow:auto;word-break:break-all;height:25px;min-width: 200px; max-height: 15px;float: left; width: 99%;padding-top:0px;' class='divscroll'>", this.objBase.SpecialDecode(sqlDataReader1["ContactNumber"].ToString()), "&nbsp;</div></td>")));
                        }
                        if (this.chkColumns.Items[3].Selected)
                        {
                        ...
                        if (this.chkColumns.Items[5].Selected)
                        {
                            this.plhdetails.Controls.Add(new LiteralControl(string.Concat("<td class='table-rowrpt' align='left' width='5%' style='vertical-align:middle'><div id='division' style='overflow:auto;word-break:break-all;height:25px;min-width: 200px; max-height: 15px;float: left; width: 99%;padding-top:0px;' class='divscroll'>", this.objBase.SpecialDecode(sqlDataReader1["CallType"].ToString()), "&nbsp;</div></td>")));
                        }
                        if (this.chkColumns.Items[6].Selected)
                        {
                            this.plhdetails.Controls.Add(new LiteralControl(string.Concat("<td class='table-rowrpt' align='left' width='5%' style='vertical-align:middle'><div id='division' style='overflow:auto;word-break:break-all;height:25px;min-width: 200px; max-height: 15px;float: left; width: 99%;padding-top:0px;' class='divscroll'>", this.objBase.SpecialDecode(sqlDataReader1["CallDetails"].ToString()), "&nbsp;</div></td>")));
                        }
                        if (this.chkColumns.Items[7].Selected)
                        {
                            this.plhdetails.Controls.Add(new LiteralControl(string.Concat("<td class='table-rowrpt' align='left' width='5%' style='vertical-align:middle'><div id='division' style='overflow:auto;word-break:break-all;height:25px;min-width: 200px; max-height: 15px;float: left; width: 99%;padding-top:0px;' class='divscroll'>", this.objBase.SpecialDecode(sqlDataReader1["CallPurpose"].ToString()), "&nbsp;</div></td>")));
                        }
                        if (this.chkColumns.Items[8].Selected)
                        {
                        ...
                        this.plhdetails.Controls.Add(new LiteralControl(string.Concat("<td class='table-rowrpt' align='left' width='5%' style='vertical-align:middle'><div id='division' style='overflow:auto;word-break:break-all;height:25px;min-width: 200px; max-height: 15px;float: left; width: 99%;padding-top:0px;' class='divscroll'>", this.objJava.Eprint_return_Date_Before_View(sqlDataReader1["DateAndTime"].ToString(), this.CompanyID, this.UserID, false), "&nbsp;</div></td>")));
                    }
                    sqlConnection.Close();
                    this.plhdetails.Controls.Add(new LiteralControl("</tr></table>"));
                    this.plhdetails.Controls.Add(new LiteralControl("</td>"));
            ...
            foreach (DataRow row1 in dataSet.Tables[0].Rows)
            {
                ...
                {
                    row1["CompanyName"] = this.objBase.SpecialDecode(row1["CompanyName"].ToString());
                }
                if (row1.Table.Columns.Contains("ContactName"))
                {
                    row1["ContactName"] = this.objBase.SpecialDecode(row1["ContactName"].ToString());
                }
                if (row1.Table.Columns.Contains("SalesPerson"))
                {
                    row1["SalesPerson"] = this.objBase.SpecialDecode(row1["SalesPerson"].ToString());
                }
                if (row1.Table.Columns.Contains("Subject"))
                {
                    row1["Subject"] = this.objBase.SpecialDecode(row1["Subject"].ToString());
                }
                if (row1.Table.Columns.Contains("CallType"))
                {
                    row1["CallType"] = this.objBase.SpecialDecode(row1["CallType"].ToString());
                }
                if (row1.Table.Columns.Contains("CallDetails"))
                {
                    row1["CallDetails"] = this.objBase.SpecialDecode(row1["CallDetails"].ToString());
                }
                if (row1.Table.Columns.Contains("CallPurpose"))
                {
                    row1["CallPurpose"] = this.objBase.SpecialDecode(row1["CallPurpose"].ToString());
                }
                if (row1.Table.Columns.Contains("AssignedTo"))
                {
                    row1["AssignedTo"] = this.objBase.SpecialDecode(row1["AssignedTo"].ToString());
                }
                if (!row1.Table.Columns.Contains("Description"))
                {
                    continue;
                }
                row1["Description"] = this.objBase.SpecialDecode(row1["Description"].ToString());
            }
        ...
        public void GetPageBind(int PageNumber)
        {
            ...
            DataSet callDetailsData = this.GetCallDetailsData(PageNumber);
            this.lblTotalRecords.Text = callDetailsData.Tables[1].Rows[0][0].ToString();
            if (callDetailsData.Tables[0].Rows.Count == 0)
            {
                this.pnlEmptyRecords.Visible = true;
        ...
        {
            DataTable dataTable = SettingsBasePage.ActivityCallReport_SelectColoumn();
            ...
            if (dataTable.Rows.Count > 1)
            {
                foreach (DataRow row in dataTable.Rows)
                {
                    ...
                    for (int j = 0; j < dataTable.Columns.Count; j++)
                    {
                        ...
                        {
                            row["ScreenName"] = this.objlang.GetLanguageConversion("Call_Start_date");
                        }
                    }
                }
        ...
        private void GridActivityCallReport_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.Header)
            {
                ...
                for (int i = 0; i < e.Row.Controls.Count; i++)
                {
                    ...
                    else if (e.Row.Cells[i].Text == "ContactNumber")
                    {
                        e.Row.Cells[i].Text = this.objLangClass.GetLanguageConversion("Contact_Number");
                    }
                    ...
                    else if (e.Row.Cells[i].Text == "CallType")
                    {
                        e.Row.Cells[i].Text = this.objLangClass.GetLanguageConversion("Call_Type");
                    }
                    else if (e.Row.Cells[i].Text == "CallDetails")
                    {
                        e.Row.Cells[i].Text = this.objLangClass.GetLanguageConversion("Call_Details");
                    }
                    else if (e.Row.Cells[i].Text == "CallPurpose")
                    {
                        e.Row.Cells[i].Text = this.objLangClass.GetLanguageConversion("Call_Purpose");
                    }
                    ...
                    {
                        e.Row.Cells[i].Text = this.objLangClass.GetLanguageConversion("Call_Start_date");
            ...
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                {
                    e.Row.Cells[this.cellvalue_DateTime].Text = this.objJava.Eprint_return_Date_Before_View(e.Row.Cells[this.cellvalue_DateTime].Text, this.CompanyID, this.UserID, false);
                }
        ...
        protected void GridCallReport_DataBound(object sender, EventArgs e)
        {
            int num = 0;
            for (int i = 0; i < this.chkColumns.Items.Count; i++)
            {
                if (this.chkColumns.Items[i].Selected && (i == 0 || i == 1 || i == 2 || i == 3))
                {
                    num++;
                }
            }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                base.Title = _languageClass.convert(global.pageTitle("Call Report", int.Parse(this.Session["companyid"].ToString()), this.Session["companyName"].ToString()));
                if (this.pagename.ToString().ToLower().Trim() != "report")
                {
                    ...
                    base.Navigation_Path(string.Concat(languageConversion), string.Concat("&nbsp;>&nbsp;", this.objLangClass.GetLanguageConversion("Call_Report")));
                }
                else
                {
                ...
                HtmlGenericControl spndlDate_AnnualYear1 = this.spndlDate_AnnualYear;
                string[] spndlDate_AnnualYear33 = new string[] { "(",
                    this.objJava.Eprint_return_Date_Before_View(
                        ePrintUtilities.Utilities.GetFirstDateOfTheCuurentYear(this.DateFormat).ToString()
                ...
                spndlDate_AnnualYear1.InnerText = string.Concat(spndlDate_AnnualYear33);
                ...
                this.ddlCalls.Items[0].Text = string.Concat("--", this.objLangClass.GetLanguageConversion("Select"), "--");
                this.ddlCalls.Items[1].Text = this.objLangClass.GetLanguageConversion("All");
                this.ddlCalls.Items[2].Text = this.objLangClass.GetLanguageConversion("Completed");
                this.ddlCalls.Items[3].Text = this.objLangClass.GetLanguageConversion("Over_Due");
                ...
                this.usrReportsave.onLinkClientreport += new LinkClientRunReportEventHandler(this.usrReportsave_onLinkClientreport);
            }
            catch
            {
            }
        ...
        private void ReportDetails(int ReportID)
        {
            ...
            foreach (DataRow row in dataSet.Tables[0].Rows)
            {
                ...
                for (int j = 0; j < (int)strArrays.Length; j++)
                {
                    ...
                    else if (strArrays[j] == "ContactNumber")
                    {
                    ...
                    else if (strArrays[j] == "CallType")
                    {
                        this.chkColumns.Items[5].Selected = true;
                    }
                    else if (strArrays[j] == "CallDetails")
                    {
                        this.chkColumns.Items[6].Selected = true;
                    }
                    else if (strArrays[j] == "CallPurpose")
                    {
                ...
                if (row["Calls"].ToString() != "")
                {
                    if (row["Calls"].ToString() == "All")
                    {
                        base.SetDDLValue(this.ddlCalls, row["Calls"].ToString());
                    }
                    else if (row["Calls"].ToString() == "Completed")
                    {
                        base.SetDDLValue(this.ddlCalls, row["Calls"].ToString());
                    }
                    else if (row["Calls"].ToString() == "OverDue")
                    {
                        base.SetDDLValue(this.ddlCalls, row["Calls"].ToString());
                    }
                }
        ...
        private void SaveReports(string value)
        {
            ...
            for (int i = 0; i < this.chkColumns.Items.Count; i++)
            {
                if (this.chkColumns.Items[i].Selected)
                {
                    if (i != 0)
                    {
                        ...
                        activity_callreport clientActivityCallreport = this;
                        clientActivityCallreport.ColumnNames = string.Concat(clientActivityCallreport.ColumnNames, "µ", str1);
                    }
                    else
                    {
                        str1 = this.chkColumns.Items[i].Value;
            ...
            if (this.ddlCalls.SelectedValue != "0")
            {
                empty = string.Concat(empty, " ,Calls");
                str = string.Concat(str, ", '", this.ddlCalls.SelectedValue, "'");
            }
            for (int j = 0; j < this.chkFreeText.Items.Count; j++)
            {
                if (this.chkFreeText.Items[j].Selected)
            ...
            object obj = str;
            object[] num = new object[] { obj, ", 'activitycallreport',", Convert.ToInt32(this.Session["companyid"].ToString()), ",", Convert.ToInt32(this.Session["UserID"].ToString()) };
        ...
        private void usrReportsave_onLinkClientreport(int ReportID)
        {
            this.Session["DeleteMsg"] = "SelectDeleteTab";
        }
        ...
        protected void GridCall_OnNeedDataSource(object source, GridNeedDataSourceEventArgs e)
        {
            if (base.IsPostBack)
            {
                DataSet ClientData = this.GetCallDetailsData(this.PageNumber);
                DataTable dt = SetCallReportColumns(ClientData);
                this.GridEstReport.DataSource = dt;
            }

        }
        protected void GridCall_PageIndexChanged(object sender, GridPageChangedEventArgs e)
        {
            this.Session["DeleteMsg"] = null;
            this.PageNumber = e.NewPageIndex + 1;
        }

        protected void GridCall_PageSizeChanged(object sender, GridPageSizeChangedEventArgs e)
        {
            this.Session["DeleteMsg"] = null;
