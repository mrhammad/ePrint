...
{
    public partial class order_printbroker : BaseClass, IRequiresSessionState
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (string.Compare(this.QueryType, "edit", true) == 0)
            {
                ...
                if (this.IsItemCompleted == 1)
                {
                    ...
                    base.Navigation_Path(string.Concat(languageConversion), string.Concat("&nbsp;", this.objLanguage.GetLanguageConversion("Order_Edit_Outwork")));
                ...
                {
                    base.Navigation_Path(string.Concat("<a href=../orders/order_view.aspx class='subnavigator'  style='text-decoration:underline;'>", this.objLanguage.GetLanguageConversion("Order_View"), "</a>"), string.Concat("&nbsp;>&nbsp;", this.objLanguage.GetLanguageConversion("Order_Add_Outwork")));
            ...
            if (string.Compare(base.Request.Params["type"].ToString(), "add", true) == 0)
            {
                ...
                if (base.Request.Params["FromAddAnItem"] != null)
                {
                    if (base.Request.Params["FromAddAnItem"].ToString() == "Y")
                    {
                        ...
                        base.Navigation_Path(string.Concat(strArrays), string.Concat("&nbsp;", this.objLanguage.GetLanguageConversion("Order_Add_Outwork")));
                    }
                }
                else if (base.Request.Params["maintype"] != "edit")
                {
                    ...
                    base.Navigation_Path(string.Concat(languageConversion1), this.objLanguage.GetLanguageConversion("Order_Add_Outwork"));
                }
                else
                {
                    ...
                    base.Navigation_Path(string.Concat(strArrays1), string.Concat("&nbsp;", this.objLanguage.GetLanguageConversion("Order_Add_Outwork")));
                }
                base.Title = this.objLanguage.convert(global.pageTitle("Order Add", int.Parse(this.Session["companyid"].ToString()), this.Session["companyName"].ToString()));
            }
        }
