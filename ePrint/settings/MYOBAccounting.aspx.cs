...
{
    public partial class MYOBAccounting : BaseClass, IRequiresSessionState
    {
        ...
        protected void btn_InvoiceExport_Click(object sender, EventArgs e)
        {
            ...
            DataTable dataTable = new DataTable();
            if (Convert.ToString(Session["CompanyName"]).Contains("Clockwork") || Convert.ToString(Session["CompanyName"]).Contains("Handy Envelopes") || Convert.ToString(Session["CompanyName"]).Contains("Vivid Print & Promotions Pty. Ltd.") || Convert.ToString(Session["CompanyName"]).Contains("Inkifingus") || Convert.ToString(Session["CompanyName"]).Contains(@"Dual Print Solutions PTY LTD T\AS Valiant Press") || Convert.ToString(Session["CompanyName"]).Contains("Brightset") || Convert.ToString(Session["CompanyName"]).Contains("Spark") || Convert.ToString(Session["CompanyName"]).Contains("iPrint Digital")
                || Convert.ToString(Session["CompanyName"]).Contains("Creative Plastic Cards") || Convert.ToString(Session["CompanyName"]).Contains("Twin Loop"))
            {
            
                // Ticket #9629 MYOB export excel file hearder names updated  Ticket# 82382 for Vivid
            ...
            string[] secureDocPath2 = new string[] { this.SecureDocPath, this.ServerName, "/", this.Session["CompanyID"].ToString(), "/AccountingExport/Myob/Myob_Invoice_", this.DateTimeForFileName.ToString().Replace("/", "-"), ".xls" };
            string str2 = string.Concat(secureDocPath2);
            Export export = new Export();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            this.header_mis.SettingName = this.objLanguage.GetLanguageConversion("MYOB_Accounting");
