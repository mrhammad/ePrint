...
namespace nmsLoginclass
{
	public class loginclass : System.Web.UI.Page
	{
		...
		public void Insert_intoResume_Session(string email, string password, long AccountID)
		{
			commonclass _commonclass = new commonclass();
			SqlCommand sqlCommand = new SqlCommand("[Ws_B2b_ResumeSessionStore_insert]", _commonclass.openConnection())
			{
				CommandType = CommandType.StoredProcedure
			};
			sqlCommand.Parameters.Add("@Email", email);
			sqlCommand.Parameters.Add("@Password", password);
			sqlCommand.Parameters.Add("@AccountID", AccountID);
		...
		public void LogIn_FromBaseClass(string email, string password, long AccountID)
		{
			foreach (DataRow row in LoginBasePage.LoginTo_Store(email, password, AccountID, ConnectionClass.AccountType).Rows)
			{
				...
				this.Session["AccountID"] = row["AccountID"].ToString();
				this.Session["AccountName"] = row["accountName"].ToString();
				...
				this.Session["DeptID"] = row["DepartmentID"].ToString();
