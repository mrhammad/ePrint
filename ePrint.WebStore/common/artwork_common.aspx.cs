...
namespace ePrint.WebStore.common
{
    public partial class artwork_common : BaseClass, IRequiresSessionState
    {
        ...
        public string OrdJobInvid = string.Empty;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                this.pagetype = base.Request.QueryString["from"].ToString();
            }
            ...
            {
                if (base.Request.QueryString["OrderID"].ToLower().Contains("job") || base.Request.QueryString["OrderID"].ToLower().Contains("inv"))
                {
                    this.OrdJobInvid = base.Request.QueryString["OrderID"].ToString();
                }
                else
                {
                    this.OrderID = Convert.ToInt64(base.Request.QueryString["OrderID"].ToString());
                }
            }
            ...
            if (base.Request.QueryString["from"] != null)
            {
                ...
                {
                    DataTable dataTable = CartBasePage.ArtworkFile_Cart_Order_Select(this.CartItemID, this.CartID, this.OrderID, "cart");
                ...
                {
                    DataTable dataTable1 = CartBasePage.ArtworkFile_Cart_Order_Select(this.CartItemID, this.OrderItemId, this.OrderID, "order");
                    this.grdFilesUploaded.DataSource = dataTable1;
                    this.grdFilesUploaded.DataBind();
                    return;
                }
                if (base.Request.QueryString["from"].ToString().ToLower() == "job" || base.Request.QueryString["from"].ToString().ToLower() == "invoice")
                {
                    DataTable dataTable2 = CartBasePage.JobInvoice_ArtworkFile_Select_splititem(this.CartItemID, this.OrderItemId);
                    this.grdFilesUploaded.DataSource = dataTable2;
                    this.grdFilesUploaded.DataBind();
                }
            }
        }
