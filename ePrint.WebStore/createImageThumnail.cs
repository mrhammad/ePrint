...
public class createImageThumnail : System.Web.UI.Page
{
	...
	public static string FixedSizeForGif(string OriginalFile, Image imgPhoto, int Width, int Height, bool OnlyResizeIfWider)
	{
		Image image = Image.FromFile(OriginalFile);
		image.RotateFlip(RotateFlipType.Rotate180FlipNone);
		image.RotateFlip(RotateFlipType.Rotate180FlipNone);
		...
		float width1 = 0f;
		float height1 = 0f;
		width1 = (float)Width / (float)width;
		height1 = (float)Height / (float)num;
		if (height1 >= width1)
		{
			single = width1;
			float single1 = ((float)Height - (float)num * single) / 2f;
		}
		else
		{
			single = height1;
			float width2 = ((float)Width - (float)width * single) / 2f;
		}
		int num1 = (int)((float)width * single);
		int num2 = (int)((float)num * single);
		return string.Concat(num1, "~", num2);
	}
